...
exports.point_fn = function(selection_obj, geo, ic, order_months, ndays_months, summr_months, wintr_months){ 
  ...
  var yyear_ic = icB.filterDate(start, end);

  var scale = ic.first().projection().nominalScale().getInfo();

  function make_pixel_im_fn(im_obj){
    var pixel_ft = im_obj.sampleRegions({collection:geo, scale:scale}).first();
    var prPixel_im = ee.Image.constant(pixel_ft.get('pr')).rename('pr').clip(geo).toFloat();
    var txPixel_im = ee.Image.constant(pixel_ft.get('tasmax')).rename('tasmax').clip(geo).toFloat();
    var tnPixel_im = ee.Image.constant(pixel_ft.get('tasmin')).rename('tasmin').clip(geo).toFloat();
    var pixel_im = ee.Image([prPixel_im, txPixel_im, tnPixel_im]).copyProperties(im_obj, ['system:time_start']);
    return pixel_im;
  }
  
  var year_ic = yyear_ic.map(make_pixel_im_fn);
